...
public static class Utils
{
    ...
    public static TimeSpan ParseDurationAsTimeSpan(string durationString)
    {
        ...
        if (GlobalVariables.Iso8601Pattern().IsMatch(durationString) && TimeSpan.TryParse(durationString.Replace("PT", "", StringComparison.OrdinalIgnoreCase), out var isoTimeSpan))
            return isoTimeSpan;

        if (GlobalVariables.TimeFormatPattern().IsMatch(durationString) && TimeSpan.TryParse(durationString, out var timeSpan))
            return timeSpan;

        if (GlobalVariables.IntegerPattern().IsMatch(durationString) && int.TryParse(durationString, out var seconds))
